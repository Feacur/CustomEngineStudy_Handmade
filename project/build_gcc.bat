...
set dll_map=-Wl,--Map=.\%intermediate%_dll.map
set dll_imp=-Wl,--out-implib,.\%intermediate%_dll.a

set exe_map=-Wl,--Map=.\%intermediate%_exe.map
...
set dll_compiler=-o %target_name%.dll %compiler%
set exe_compiler=-o %target_name%.exe %compiler%

echo ---- DLL  ---- %time%
gcc "..\code\game.cpp"  %dll_compiler% -shared %dll_imp%
echo ---- EXE  ---- %time%
gcc "..\code\win32.cpp" %exe_compiler%         %libs% %subsystem%
echo ---- DONE ---- %time%
