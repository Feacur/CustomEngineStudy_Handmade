...
inline uint8 get_mask_offset(uint32 value) {
	uint8 bits_in_value = 8 * sizeof(value);
	for (uint8 i = 0; i < bits_in_value; ++i) {
...
RGBA_Data read_bitmap_image(Memory_Chunk *memory, uint8 * bytes) {
	...
	int32 pixels_count = info_header->width * info_header->height;
	
	RGBA_Data result = {};
	result.size = {info_header->width, info_header->height};
	
	result.data = ALLOCATE_ARRAY(memory, uint32, pixels_count);
	memcpy(result.data, bytes + file_header->data_offset, sizeof(uint32) * pixels_count)
...
RGBA_Data read_bitmap_file(cstring file_name, read_file_func * read_file) {
	uint8 * bytes = read_file(&globals::transient_memory, file_name);
	if (bytes) {
