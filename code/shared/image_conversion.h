inline float get_color32_component_as_float(uint32 color32, uint8 offset) {
	uint32 component = (color32 >> offset) & 0xff;
	return (float)component / 0xff;
}

inline Vector4 color32_to_vector4(uint32 color32, uint8 offsets[4]) {
	return {
...
inline uint32 vector4_to_color32(Vector4 color, uint8 offsets[4]) {
